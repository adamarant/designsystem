 ...
 * </AdminPage>
 */
export function AdminPage({ title, description, onBack, backLabel, actions, titleClassName, search, filters, toolbarActions, toolbar, status = 'ready', empty, loadingLabel, pagination, footer, overlays, children, className, }) {
    ...
    let body;
    if (status === 'loading') {
        body = _jsx(AdminSpinner, { label: loadingLabel });
    }
    else if (status === 'empty' && empty) {
        body = _jsx(AdminEmptyState, { variant: "card", ...empty });
    }
    else {
        body = children;
    }
    const showFooter = footer !== undefined || (pagination !== undefined && status === 'ready');
    return (_jsxs(_Fragment, { children: [_jsxs("div", { className: className ? `ds-admin-page ${className}` : 'ds-admin-page', children: [hasHeader && (_jsx(AdminPageHeader, { title: title, description: description, onBack: onBack, backLabel: backLabel, actions: actions, titleClassName: titleClassName })), showToolbar && (_jsx("div", { className: "ds-admin-page__toolbar", children: toolbar ?? (_jsx(AdminToolbar, { search: search, filters: filters, actions: toolbarActions })) })), body !== undefined && body !== null && (_jsx("div", { className: "ds-admin-page__body", children: body })), showFooter && (_jsx("div", { className: "ds-admin-page__footer", children: footer ?? (pagination && _jsx(AdminPagination, { ...pagination })) }))] }), overlays] }));
}
